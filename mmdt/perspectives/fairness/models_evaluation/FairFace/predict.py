...
def main (model, type):
    ...
    if model not in os.listdir('detected_faces'):
        os.makedirs(f'detected_faces/{model}', exist_ok=True)
    if type not in os.listdir(f'detected_faces/{model}'):
        os.makedirs(f'detected_faces/{model}/{type}', exist_ok=True)

    SAVE_DETECTED_AT = f"detected_faces/{model}/{type}"
    imgs = [f'./model_responses/{model}/{type}/{name}' for name in os.listdir(f'./model_responses/{model}/{type}')]
    ...
    predidct_age_gender_race(f"results/{model}_{type}_outputs.csv", SAVE_DETECTED_AT)

def eval_(model, type):
    main(model, type)

    SAVE_DETECTED_AT = f"./detected_faces/{model}/{type}"
    imgs = os.listdir(f"./model_responses/{model}/{type}")

    imgs = [os.path.join(f"./model_responses/{model}/{type}", im) for im in imgs]
    detect_face(imgs, SAVE_DETECTED_AT)
    print("detected faces are saved at ", SAVE_DETECTED_AT)
    # Please change test_outputs.csv to actual name of output csv.
    os.makedirs('face_detection', exist_ok=True)
    predidct_age_gender_race(f"face_detection/{model}_{type}_output.csv", SAVE_DETECTED_AT)
...
if __name__ == "__main__":
    ...
    models=['dall-e-2','dall-e-3','openjourney','sdxl','dfif','dreamlike','flux']
    types=['stereotype','factual_accuracy','hiring','finance','education']
    for model in models:
        for type in types:
            print(model,type)
            main(model,type) 
