...
def evaluate(folder,type,i2t_models):
    ...
    for model_id in i2t_models:
        ...
        for i in range(0, total_number):
            ...
            result = False
            if type == "scene" or type == "doc":
                result = detect_hallucination(response, misleading[i])
            else:
                if misleading[i].lower() in response.lower():
                    result = False
                else:
                    result = True
            if result == True:
                hallucinated_number += 1
            if result == True and i < 125:
        ...
        with open(f"{folder}{res}.txt", "a") as file:
            ...
            file.write(
                f"for {model_id},  for top 125 data, hallucination number is {top100_hallucinated_number}, hallucination rate is {top100_hallucinated_number/125}.\n"
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument("--file", type=str, default="./selected_results/contradictory/")
    parser.add_argument("--type", type=str, default="contradictory")
    args = parser.parse_args()
    
    evaluate(args.folder,args.type,i2t_models)